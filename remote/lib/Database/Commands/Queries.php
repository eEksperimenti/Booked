...
class Queries
{
	...
	const ADD_RESERVATION_SERIES =
			'INSERT INTO
        reservation_series (date_created, title, description, allow_participation, allow_anon_participation, repeat_type, repeat_options, type_id, status_id, owner_id,experiment)
		VALUES (@dateCreated, @title, @description, @allow_participation, false, @repeatType, @repeatOptions, @typeid, @statusid, @userid,@experiment)';
	...
	const GET_NEXT_RESERVATIONS = 'SELECT  ri.*, rs.title,rs.experiment, rs.description, rr.resource_id, ru.user_id, MIN(ri.start_date)
...
class QueryBuilder
{
	...
	public static $SELECT_LIST_FRAGMENT = 'ri.*,rs.experiment, rs.date_created as date_created, rs.last_modified as last_modified, rs.description as description, rs.status_id as status_id, rs.title, rs.repeat_type, rs.repeat_options,
